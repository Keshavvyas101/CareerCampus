...
export const FeedbackCard = ({file, loading, feedback}) => {
  const atsMatch = feedback ? feedback.match(/ATS Score:\s*(\d+)\/100/i) : null;
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-6 relative min-h-[300px] max-h-[700px] overflow-auto">
      ...
      {loading ? (
        ...
        <>
          {atsScore && (
            <div className="flex flex-col items-center justify-center mb-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
              <span className="text-sm font-semibold text-blue-800 uppercase tracking-wider">
                ATS Score
              </span>
              <div className="flex items-baseline mt-1">
                <span className="text-4xl font-extrabold text-blue-600">{atsScore}</span>
                <span className="text-xl text-blue-400 font-semibold">/100</span>
              </div>
              <div className="w-full bg-blue-200 rounded-full h-2 mt-3 max-w-[240px]">
                <div
          ...
          {bulletPoints.length > 0 ? (
            ...
            <p className="text-gray-700 whitespace-pre-line">
              {feedback.replace(/ATS Score:\s*\d+\/100/i, "").trim()}
            </p>
          )}
        </>
      ) : (
